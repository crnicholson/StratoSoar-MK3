...
new_t_lat = 0  # 0 by default means glider uses programmed ta`rget location.
new_t_lon = 0
...
def input_handler():
    global new_t_lon, new_t_lat, t_lat, t_lon, abort, receivedAbort
    ...
    while True:
        try:
            ...
            command = input("Enter command (set_lat_lon/abort): \n").strip()
            if command == "set_lat_lon":
                ...
                # abortCounter += 1
                # print(f"Debug: abortCounter is now {abortCounter}")
                # if abortCounter == 1:
                #     print("\nWARNING: aborting is irreversible.\n")
                #     print("For safety, remember to run the abort command twice to abort.")
                # elif abortCounter == 2:
                #     print("\nAbort signal sending on next ground station request.\n")
                #     abort = 1
                # else:
                #     print("Abort signal has already been sent.")
                if abort == 1:
                    print(
                    ...
                    command = input(
                        'Aborting is irreversible. Are you sure you want to continue? Type "abort" again to truly abort. Type anything else to cancel.\n'
                    ).strip()
                    if command == "abort":
                        print("Aborting the glider on next ground station beacon.")
                        abort = 1
                        print(abort)
                    else:
...
def change_target():
    received = request.get_json()
    global new_t_lat, new_t_lon, abort
    if received["callsign"] == call_sign:
        if new_t_lat == 0 or new_t_lon == 0:
            print(
                "Latitude or longitude equal 0. Glider will use current target location."
