...
void sendToSondehub() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin(client, String(SERVER_NAME) + "/upload-station-data");
...
void sendToGlider() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin(client, String(SERVER_NAME) + "/change-glider-data");
    http.addHeader("Content-Type", "application/json");
    ...
    if (httpResponseCode > 0) {
      ...
      if (!error) {
        ...
        if (abortFlight || (newTLon != receivedData.tLon && newTLat != receivedData.tLat)) {
#ifdef HAMMING
...
#endif
#ifndef HAMMING
          dataToSend.lat = newTLat;
          dataToSend.lon = newTLon;
          dataToSend.abortStatus = abortFlight;

          LoRa.beginPacket();
          LoRa.write(dataToSend, sizeof(dataToSend));
          LoRa.endPacket(true); // Send in async mode.
#endif
        }
      } else {
